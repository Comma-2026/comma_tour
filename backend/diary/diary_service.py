...
def create_diary(payload: dict) -> dict:
    ...
    if diary_model.get_diary_by_pin(pin_id) is not None:
        return update_diary(pin_id, payload)
...
def update_diary(pin_id: str, payload: dict) -> dict:
    ...
    updated = diary_model.update_diary(pin_id, fields)
    if updated is None:
